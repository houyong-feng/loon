...
!(async () => {
  ...
  console.log('5G超级盲盒，可抽奖获得京豆，建议在凌晨0点时运行脚本，白天抽奖基本没有京豆，3小时运行一次收集热力值\n' +
      ...
      '更新时间：2021-8-8 19:00');
  $.http.get({url: 'https://purge.jsdelivr.net/gh/chinnkarahoi/sc@master/shareCodes/jd_shareCodes.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
  await $.wait(1000)
  await updateShareCodesCDN('https://cdn.jsdelivr.net/gh/chinnkarahoi/sc@master/shareCodes/jd_shareCodes.json')
  await $.wait(1000)
  await getShareCode()
...
function getShareCode() {
  return new Promise(resolve => {
    $.get({
      url: "https://raw.fastgit.org/chinnkarahoi/sc/master/shareCodes/jd_mohe.json",
      headers: {
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/87.0.4280.88"
      }
    }, async (err, resp, data) => {
