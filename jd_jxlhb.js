...
==============Loon==============
[Script]
cron "11 3,9,10,11,12,16,21 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxlhb.js,tag=京喜领88元红包
...
!(async () => {
  ...
  console.log('京喜领88元红包\n' +
      ...
      '助力逻辑：先自己京东账号相互助力，如有剩余助力机会，则助力作者\n' +
      '温馨提示：如提示助力火爆，可尝试寻找京东客服')
  let res = await getAuthorShareCode('https://raw.githubusercontent.com/chinnkarahoi/sc/master/shareCodes/jxhb.json')
  if (!res) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/chinnkarahoi/sc@master/shareCodes/jxhb.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
