...
!(async () => {
  ...
  $.authorTuanList = await getAuthorShareCode('https://raw.githubusercontent.com/chinnkarahoi/sc/master/shareCodes/jd_zz.json');
  if (!$.authorTuanList) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/chinnkarahoi/sc@master/shareCodes/jd_zz.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorTuanList = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/chinnkarahoi/sc@master/shareCodes/jd_zz.json') || [];
  }
  const temp = await getAuthorShareCode('https://raw.fastgit.org/chinnkarahoi/sc/master/shareCodes/jd_zz.json') || []
