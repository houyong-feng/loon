...
!(async () => {
  ...
  $.updatePkActivityIdRes = await getAuthorShareCode('https://raw.githubusercontent.com/chinnkarahoi/sc/master/shareCodes/jd_cityShareCodes.json')
  if (!$.updatePkActivityIdRes) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/chinnkarahoi/sc@master/shareCodes/jd_cityShareCodes.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.updatePkActivityIdRes = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/chinnkarahoi/sc@master/shareCodes/jd_cityShareCodes.json')
  }
  let tempcode = []
